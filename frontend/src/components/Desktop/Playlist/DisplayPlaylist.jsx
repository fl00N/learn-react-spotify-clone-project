...
import MobilePlaylistImage from "./MobilePlaylistImage";

const DisplayPlaylist = () => {
  const { id } = useParams();
  const [playlistData, setPlaylistData] = useState(null);
  const [hoveredIndex, setHoveredIndex] = useState(null);
  const { playWithId, track, setNavigationToPlaylist } =
    useContext(PlayerContext);
  const { playlistsData, removePlaylist, removeSongFromPlaylist } =
  ...
  return playlistData ? (
    <div className="max-md:min-h-[calc(100vh+110px)]">
      <div className="max-md:mb-5">
        <div className="md:mt-10 flex md:gap-8 flex-col md:flex-row md:items-end">
          <div className="max-md:flex max-md:justify-center max-md:mb-5 max-md:mt-10">
            {!isMobile ? (
              ...
              <MobilePlaylistImage playlistData={playlistData} />
            )}
          </div>
          <div className="flex flex-col max-md:ml-4">
            <p className="max-md:hidden">Playlist</p>
            <h2
              onClick={
                !isMobile ? () => openEditModal(playlistData) : undefined
              }
              className="font-[Metropolis] text-xl font-bold md:mb-4 md:text-7xl md:cursor-pointer max-md:text-white"
            >
              {playlistData.name}
            </h2>
            <div className="mt-1 font-[Metropolis] font-medium text-neutral-400 max-md:text-sm">
            ...
            {isOpen && (
              <div className="absolute left-[-0.5rem] mt-4 w-[12.5rem] bg-[#282828] rounded shadow-[0_16px_24px_rgba(0,0,0,0.2)] z-[1]">
                <ul className="py-2 px-1">
                  ...
                  >
                    <img
                      className="w-4 mr-4 brightness-75"
        ...
        >
          ...
          {hoveredIndex === index && (
            <img
              ...
              className="max-md:hidden w-4 brightness-150 hover:brightness-100 right-[90px] absolute"
